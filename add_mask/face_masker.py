...
from add_mask.utils import read_info
from add_mask.model.prnet import PRNet
from add_mask.utils.cython.render import render_cy
...
class PRN:
    ...
    def __init__(self, model_path):
        ...
        self.face_ind = np.loadtxt('add_mask/Data/uv-data/face_ind.txt').astype(np.int32)
        self.triangles = np.loadtxt('add_mask/Data/uv-data/triangles.txt').astype(np.int32)
        self.net = PRNet(3, 3)
...
class FaceMasker:
    ...
    def __init__(self, is_aug):
        ...
        Args:
            is_aug(bool): whether or not to add some augmentaion operation on the mask.
        """
        self.uv_face_path = 'add_mask/Data/uv-data/uv_face_mask.png'
        self.mask_template_folder = 'add_mask/Data/mask-data'
        self.prn = PRN('add_mask/model/prnet.pth')
    ...
    def add_mask_one(self, image, face_lms, template_name):
        """Add mask to one image. Return the masked image.

        Args:
            image(np.ndarray): the image to add mask.
